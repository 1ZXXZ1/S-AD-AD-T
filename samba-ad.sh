...
# ============================================================
#        Настройка BIND9 для BIND9_DLZ (отдельная функция)
# ============================================================

configure_bind9_dlz() {
    # Определяем подсеть из HOST_IP (для allow-query и allow-recursion)
    local bind_subnet="${HOST_IP%.*}.0/24"

    # Формируем список forwarders для BIND
    local bind_forwarders=""
    for fwd in ${DNS_FORWARDERS}; do
        bind_forwarders="${bind_forwarders} ${fwd};"
    done

    info "Параметры BIND9: подсеть=${bind_subnet}, forwarders={${bind_forwarders}}"

    # 1. Отключить chroot
    control bind-chroot disabled 2>/dev/null || true
    ok "bind-chroot отключён"

    # 2. Отключить KRB5RCACHETYPE
    if [[ -f /etc/sysconfig/bind ]]; then
        grep -q KRB5RCACHETYPE /etc/sysconfig/bind 2>/dev/null || \
            echo 'KRB5RCACHETYPE="none"' >> /etc/sysconfig/bind
        ok "KRB5RCACHETYPE=none установлен в /etc/sysconfig/bind"
    fi

    # 3. Закомментировать строки в /etc/bind/local.conf (предотвращает конфликт зон)
    # Samba DLZ сам создаст зону — если local.conf тоже её определяет, bind упадёт:
    #   samba_dlz: Failed to configure zone 'test.alt'
    #   loading configuration: already exists
    if [[ -f /etc/bind/local.conf ]]; then
        # Сохраняем бэкап
        cp /etc/bind/local.conf /etc/bind/local.conf.bak."$(date +%Y%m%d%H%M%S)" 2>/dev/null || true
        # Комментируем все незакомментированные строки
        sed -i 's/^[^#]/#&/' /etc/bind/local.conf
        ok "/etc/bind/local.conf закомментирован (предотвращение конфликта зон)"
    fi

    # 4. Полная перезапись /etc/bind/options.conf по руководству «Альт Домен»
    # ВАЖНО: Всегда перезаписываем файл целиком — частичное редактирование ненадёжно.
    # Если tkey-gssapi-keytab уже присутствует, но отсутствуют forwarders,
    # listen-on, allow-query — DNS НЕ будет работать.
    if [[ -f /etc/bind/options.conf ]]; then
        # Резервное копирование оригинала
        cp /etc/bind/options.conf /etc/bind/options.conf.bak."$(date +%Y%m%d%H%M%S)" 2>/dev/null || true

        # Проверяем: существует ли файл resolvconf-options.conf
        # Если его нет — include вызовет ошибку парсинга и bind не запустится
        local resolvconf_opt_file="/etc/bind/resolvconf-options.conf"
        local include_resolvconf=""
        if [[ -f "${resolvconf_opt_file}" ]]; then
            include_resolvconf="include \"${resolvconf_opt_file}\";"
        else
            # Создаём пустой файл, чтобы include не ломал парсинг
            touch "${resolvconf_opt_file}"
            ok "Создан пустой ${resolvconf_opt_file}"
            include_resolvconf="include \"${resolvconf_opt_file}\";"
        fi

        cat > /etc/bind/options.conf << OPTSEOF
options {
        version "unknown";
        ...
        // отключает использование PID-файла
        pid-file none;

        // интеграция с Kerberos для динамического обновления DNS
        ...
        allow-query { localnets; ${bind_subnet}; };
        allow-recursion { localnets; ${bind_subnet}; };
...
OPTSEOF
        ok "/etc/bind/options.conf перезаписан для BIND9_DLZ"
    else
        warn "Файл /etc/bind/options.conf не найден — настройте BIND9 вручную"
    fi

    # 5. Создать override для bind.service (LDB_MODULES_DISABLE_DEEPBIND)
    # Без этого bind может падать на ALT Linux из-за конфликта загрузки модулей LDB
    local bind_override_dir="/etc/systemd/system/bind.service.d"
    local bind_override="${bind_override_dir}/override.conf"
    if [[ ! -f "${bind_override}" ]]; then
        mkdir -p "${bind_override_dir}"
        cat > "${bind_override}" << EOF
[Service]
Environment="LDB_MODULES_DISABLE_DEEPBIND=1"
EOF
        systemctl daemon-reload 2>/dev/null || true
        ok "bind.service override (LDB_MODULES_DISABLE_DEEPBIND=1) создан"
    fi

    # 6. Удалить старый include DLZ из named.conf (на случай повторной установки)
    # Будет добавлен заново после provision, когда файл /var/lib/samba/bind-dns/named.conf появится
    sed -i '/bind-dns/d' /etc/bind/named.conf 2>/dev/null || true

    # Остановить bind — будет запущен после provision с подключённым DLZ
    systemctl stop bind 2>/dev/null || true
    ok "BIND9 подготовлен (остановлен до provision)"
}
...
EOF
    ...
    # Примечание: для BIND9_DLZ форвардеры настраиваются в /etc/bind/options.conf,
    # а параметр "dns forwarder" в smb.conf игнорируется. Но передаём его для совместимости.
...
EOF
    ...
    if [[ "${DNS_BACKEND}" == "BIND9_DLZ" ]]; then
        # Подключить плагин BIND_DLZ в /etc/bind/named.conf
        # Файл /var/lib/samba/bind-dns/named.conf создаётся при provision
        if [[ -f /var/lib/samba/bind-dns/named.conf ]]; then
            # Удаляем старый include (на случай повторной установки)
            sed -i '/bind-dns/d' /etc/bind/named.conf 2>/dev/null || true
            # Добавляем include
            echo 'include "/var/lib/samba/bind-dns/named.conf";' >> /etc/bind/named.conf
            ok "Плагин BIND_DLZ подключён в /etc/bind/named.conf"
        else
            warn "Файл /var/lib/samba/bind-dns/named.conf не найден — BIND_DLZ не подключён"
            warn "Проверьте, что samba-tool domain provision завершился успешно"
        fi

        # Проверка конфигурации bind перед запуском
        if command -v named-checkconf &>/dev/null; then
            local checkconf_out
            checkconf_out="$(named-checkconf 2>&1)" || true
            if [[ -n "${checkconf_out}" ]]; then
                warn "named-checkconf обнаружил ошибки:"
                echo "${checkconf_out}"
                warn "Попытка запуска bind в любом случае..."
            else
                ok "named-checkconf: конфигурация BIND корректна"
            fi
        fi

        # Права доступа к dns.keytab (named должен читать для GSS-TSIG)
        if [[ -f /var/lib/samba/bind-dns/dns.keytab ]]; then
            chown root:named /var/lib/samba/bind-dns/dns.keytab 2>/dev/null || true
            chmod 640 /var/lib/samba/bind-dns/dns.keytab 2>/dev/null || true
            ok "Права dns.keytab настроены (named:read)"
        fi

        # Запустить bind
        systemctl enable --now bind || { error "Не удалось запустить bind"; exit 1; }
        info "Ожидание загрузки bind и подключения DLZ-плагина (5 сек)..."
        sleep 5

        # Проверяем что bind реально работает
        if ! systemctl is-active bind &>/dev/null; then
            warn "Служба bind не активна, пробуем перезапуск..."
            systemctl restart bind || { error "Не удалось перезапустить bind"; exit 1; }
            sleep 3
        fi

        # Проверяем, что DLZ-зона загружена (ищем в журнале bind)
        if systemctl is-active bind &>/dev/null; then
            local bind_log
            bind_log="$(journalctl -u bind --no-pager -n 50 2>/dev/null)" || true
            if echo "${bind_log}" | grep -qi 'samba_dlz.*configured.*zone'; then
                ok "DLZ-плагин: зона AD DNS загружена в BIND"
            elif echo "${bind_log}" | grep -qi 'samba_dlz.*Failed\|already exists'; then
                error "BIND: ошибка загрузки DLZ-зоны! Проверьте: journalctl -u bind"
                error "Возможная причина: конфликт зон в /etc/bind/local.conf"
            else
                info "Проверьте журнал bind: journalctl -u bind -n 30"
            fi
        fi

        # Быстрая проверка DNS — если не работает, перезапускаем bind
        local dns_quick_ok=0
        for _dns_try in 1 2 3; do
            if host "${DOMAIN}" 127.0.0.1 &>/dev/null; then
                dns_quick_ok=1
                break
            fi
            info "Ожидание готовности DNS (попытка ${_dns_try}/3)..."
            sleep 2
        done
        if [[ "${dns_quick_ok}" -eq 0 ]]; then
            warn "DNS на 127.0.0.1 не отвечает — перезапуск bind..."
            systemctl restart bind 2>/dev/null || true
            sleep 5
        fi
    fi

    # ВНИМАНИЕ: НЕ перезапускаем сеть после запуска bind!
    # systemctl restart network ломает loopback-слушатель bind
    # (creating IPv4 interface lo failed; interface ignored),
    # после чего DNS-запросы на 127.0.0.1 не обрабатываются.
    # DNS-настройки уже применены через resolvconf -u (шаг 5).
    ok "Службы запущены"
...
do_status() {
    ...
    if [[ "${DNS_BACKEND:-}" == "BIND9_DLZ" ]]; then
        ...
        # Проверка named.conf — подключён ли DLZ-плагин
        if grep -q 'bind-dns' /etc/bind/named.conf 2>/dev/null; then
            ok "DLZ-плагин подключён в /etc/bind/named.conf"
        else
            warn "DLZ-плагин НЕ подключён в /etc/bind/named.conf"
            warn "Добавьте: include \"/var/lib/samba/bind-dns/named.conf\";"
            all_ok=0
        fi

        # Проверка options.conf — ключевые параметры
        if [[ -f /etc/bind/options.conf ]]; then
            local opt_issues=0
            if ! grep -q 'tkey-gssapi-keytab' /etc/bind/options.conf 2>/dev/null; then
                warn "options.conf: отсутствует tkey-gssapi-keytab"
                opt_issues=1
            fi
            if ! grep -q 'forward first\|forwarders' /etc/bind/options.conf 2>/dev/null; then
                warn "options.conf: отсутствуют forwarders — внешние DNS-запросы НЕ будут перенаправлены"
                opt_issues=1
            fi
            if ! grep -q 'allow-query' /etc/bind/options.conf 2>/dev/null; then
                warn "options.conf: отсутствует allow-query — DNS-запросы могут отклоняться"
                opt_issues=1
            fi
            if [[ "${opt_issues}" -eq 0 ]]; then
                ok "options.conf: ключевые параметры BIND9_DLZ присутствуют"
            else
                all_ok=0
            fi
        fi
    ...
    # ВАЖНО: явно указываем 127.0.0.1 как DNS-сервер для надёжности
    if command -v host &>/dev/null; then
        local realm_lower
        realm_lower="$(echo "${REALM:-unknown}" | tr '[:upper:]' '[:lower:]')"

        # Проверка A-записи домена (с повторными попытками)
        local dns_resolved=0
        for _dns_attempt in 1 2 3; do
            if host "${realm_lower}" 127.0.0.1 &>/dev/null; then
                dns_resolved=1
                break
            fi
            [[ "${_dns_attempt}" -lt 3 ]] && sleep 2
        done
        if [[ "${dns_resolved}" -eq 1 ]]; then
            ok "DNS-запись домена ${realm_lower} разрешается"
        else
            warn "DNS-запись домена ${realm_lower} НЕ разрешается (127.0.0.1)"
            if [[ "${DNS_BACKEND:-}" == "BIND9_DLZ" ]]; then
                warn "  Проверьте: journalctl -u bind — есть ли ошибка загрузки DLZ-зоны"
                warn "  Проверьте: named-checkconf — корректность конфигурации BIND"
                warn "  Проверьте: cat /etc/bind/options.conf — наличие forwarders и allow-query"
                warn "  Проверьте: ls -la /var/lib/samba/bind-dns/dns.keytab — права named"
            fi
            all_ok=0
        fi

        # SRV-записи (также с явным указанием сервера)
        if host -t SRV "_kerberos._udp.${realm_lower}." 127.0.0.1 &>/dev/null; then
        ...
        if host -t SRV "_ldap._tcp.${realm_lower}." 127.0.0.1 &>/dev/null; then
...
do_delete() {
    ...
    # --- Шаг 4b: Очистка BIND9_DLZ (если использовался) ---
    if [[ "${DNS_BACKEND}" == "BIND9_DLZ" ]]; then
        # Удалить include из named.conf
        sed -i '/bind-dns/d' /etc/bind/named.conf 2>/dev/null || true
        # Восстановить options.conf из бэкапа (если есть)
        local latest_backup
        latest_backup="$(ls -t /etc/bind/options.conf.bak.* 2>/dev/null | head -1)"
        if [[ -n "${latest_backup}" ]]; then
            cp "${latest_backup}" /etc/bind/options.conf
            ok "/etc/bind/options.conf восстановлен из бэкапа"
        fi
        # Восстановить local.conf из бэкапа (если есть)
        local local_backup
        local_backup="$(ls -t /etc/bind/local.conf.bak.* 2>/dev/null | head -1)"
        if [[ -n "${local_backup}" ]]; then
            cp "${local_backup}" /etc/bind/local.conf
            ok "/etc/bind/local.conf восстановлен из бэкапа"
        fi
        # Удалить override bind.service
        rm -f /etc/systemd/system/bind.service.d/override.conf 2>/dev/null || true
        systemctl daemon-reload 2>/dev/null || true
    fi
